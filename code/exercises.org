...
amount of radiation for a given (cubic) region:
...
1. Develop a general strategy to (numerically) evaluate integrals of the form:
 ...
 $$\sum_{i = 1}^n \int_V \frac{1}{||\vec{x} - \vec{s}_i||} ~ d \vec{x}$$
...
3. What if we are interested in splitting $V$ along, say, the $x\text{-axis}$
   into two parts that receive the same total radiation. In other words, we want
   to find a $x_1 < c < x_2$ so that:
   $$\int_{x_1}^c \int_{y_1}^{y_2}  \int_{z_1}^{z_2} \frac{1}{||\vec{x} - \vec{s}_i||}~dz~dy~dx = \int_{c}^{x_2}  \int_{y_1}^{y_2} \int_{z_1}^{z_2} \frac{1}{||\vec{x} - \vec{s}_i||}~dz~dy~dx.$$
...
4. Report on the efficiency and quality of your method.
