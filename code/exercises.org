...
1. In the =code/week12= folder you can find an implementation of =range(n)= which is a
   virtual container from $\{0, ..., n - 1 \}$. In Python, a range is defined
   as:
   #+BEGIN_SRC python
   range(start, stop[, step])
   #+END_SRC
   The behaviour of a range is documented [[https://docs.python.org/3/library/stdtypes.html#typesseq-range][on the official python.org website]].
...
2. Next, we want take the range function and implement for higher dimensions.
